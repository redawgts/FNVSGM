...
submenuSGHandler:
	...
	Else If A_ThisMenuItem = Scan For New Profiles
	{
		...
		MsgBox, Note that your actual AUTO- and QUICKSAVE will stay in the STANDARD profile.`n`nTo move it to your character's profile:`n`n*Activate STANDARD profile`n*Run Fallout New Vegas and load your AUTO/QUICKSAVE`n*Save as New Savegame`n*Run FNVSGM-Scan again

		gosub subScanForNewProfiles

		;Activate STANDARD profile
		IniWrite, STANDARD, %FNVSGMINIFILENAME%, general, active
		ACTIVE = STANDARD
	...
	else if A_ThisMenuItem = Open Fallout New Vegas Savegame Folder
	{
		If SAVEGAMESFOLDER != ""
		{
...
submenuADVHandler:
	...
	if A_ThisMenuItem = Change Play-button link...
	{
		...
		If ErrorLevel = 0
		{
			IniWrite, %strRunfile%, %FNVSGMINIFILENAME%, advanced, playbuttonlink
		}
	}
...
;	{
;		Run, FNVSGMhelpfile.html
;	}

;	else
    if A_ThisMenuItem = About
	{
		MsgBox, 0, About Fallout New Vegas Savegame Manager, Fallout New Vegas Savegame Manager`nVersion %myVERSION%`n`nAuthor: RedawgTS`nSpecial thanks to: -digitalfun-`n`nFNVSGM is published exclusively on: `nhttp://www.newvegasnexus.com/,
	}
...
submenuTrayHandler:
	...
	If A_ThisMenuItem = Run Fallout New Vegas
	{
		gosub subRunFalloutNV
	}

	Else If A_ThisMenuItem = Open FNVSGM window
	{
		Gui, Show
	}
